...
**Fixed (2026-09-17, follow-up):**
- `log_activity` originally accepted an arbitrary `p_action text` / `p_entity_type text` with no allowlist. Since the caller must already be `is_admin()`, this was never a privilege-escalation issue — an admin can already do anything an admin can do — but it meant a client-side bug (or a future careless call site) could write a fabricated activity-log row with an action/entity_type that doesn't correspond to anything real, silently corrupting the one audit trail this app relies on. Fixed by adding an explicit allowlist matching every `(action, entity_type)` combination actually produced by the codebase's `sb.rpc('log_activity', ...)` call sites (traced via `grep` across `js/candidates.js`, `js/settings-manage.js`, `js/tps-manage.js`, `js/export.js`, cross-checked against the distinct `(action, entity_type)` pairs already present in `activity_logs`):
  - actions: `export_generated`, `settings_changed`, `tps_added`, `tps_updated`, `tps_deleted`, `candidate_added`, `candidate_updated`, `candidate_deleted`
  - entity_types: `overall_summary`, `per_tps_result`, `activity_log`, `pdf_report`, `elections`, `tps`, `candidates`

  Applied directly to the live project (`create or replace function`, same body plus two `if ... not in (...) then raise exception ...` guards, mirroring `set_tps_status`'s existing pattern). Verified: a real call (`log_activity(..., 'export_generated', 'overall_summary', null)`) still inserts correctly; a bogus action (`'fake_action'`) is now rejected with `invalid action` before it ever reaches the `insert`. Note this allowlist intentionally does NOT include the actions inserted by other RPCs directly (`vote_added`, `tps_counting_started`, `snapshot_created`, `tally_bulk_adjusted`, etc.) — those never go through `log_activity`, they insert into `activity_logs` inline within their own function bodies, so they were never in scope for this allowlist.
...
| 2 | Other RPCs checked for the same input-shape gap | No other RPC has this gap. `log_activity`'s missing action/entity_type allowlist fixed as a follow-up (2026-09-17); `create_snapshot`'s election-existence check and note/reason length caps left as low-severity, not fixed |
